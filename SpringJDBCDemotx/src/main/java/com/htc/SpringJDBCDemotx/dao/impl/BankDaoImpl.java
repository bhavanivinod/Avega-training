package com.htc.SpringJDBCDemotx.dao.impl;
...
import com.htc.SpringJDBCDemotx.dao.mapper.AccountRowMapper;
...
public class BankDaoImpl implements BankDao {
	...
	public void withdraw(Account fromAccount, Account toAccount, Double amount) throws InsufficientAccountBalanceException {
		...
		if (accountFromDb.getAccountBalance() >= amount) {
			String SQL = "UPDATE citi_bank set account_balance=? WHERE account_no=?";
	...
	private Account getAccountFromDb(Long accountNumber) {
		String SQL ="SELECT * FROM citi_bank WHERE account_no = ?";
